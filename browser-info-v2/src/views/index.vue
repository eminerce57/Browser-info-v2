...
const handleMouseMove = (event) => {
  TrackMouse.value = `${event.x} - ${event.y}`;
};
const TrackMouse = ref("");
const info = reactive({
  ...
  HardwareInformation: getHardwareInfo(),
});
...
<template>
  <div class="body" @mousemove="handleMouseMove">
    <Card style="width: 65rem; overflow: hidden">
      ...
      <template #subtitle
        ><strong>Browser Name: </strong>{{ info?.BrowserName }}
      </template>
      <template #content>
        <div class="grid">
          <div class="col-12">
            <Tag severity="info" rounded>Browser Version:</Tag>
            {{ info?.BrowserVersion }}
          </div>
          <div class="col-12">
          ...
          <div class="col-3">
            <Tag severity="info" rounded>User Language:</Tag>
            {{ info?.UserLanguage }}
          </div>
          <div class="col-3">
            <Tag severity="info" rounded>Java Enabled:</Tag>
            {{ info?.JavaEnabled }}
          </div>
          <div class="col-3">
            <Tag severity="info" rounded>Cookies Enabled:</Tag>
            {{ info?.CookiesEnabled }}
          </div>
          <div class="col-3">
            <Tag severity="info" rounded>Screen:</Tag>
            {{ info?.ScreenWidth }}X{{ info?.ScreenHeight }}
          </div>
          <div class="col-3">
            <Tag severity="info" rounded>Color Depth:</Tag>
            {{ info?.ScreenColorDepth }}
          </div>
          <div class="col-3">
            <Tag severity="info" rounded>Do Not Track:</Tag>
            {{ info?.DoNotTrack }}
          </div>
          <div class="col-3">
          ...
          <div class="col-3">
            <Tag severity="info" rounded>Hardware Concurrency:</Tag>
          ...
          <div class="col-3">
            <Tag severity="info" rounded>Time Zone:</Tag> GMT+0{{
              info?.TimeZone
            }}:00
          </div>
          <div class="col-3">
            <Tag severity="info" rounded>Touch Points:</Tag>
            {{ info?.TouchPoints }}
          </div>
          <!-- <div class="col-3">
          ...
          <div class="col-4">
            <Tag severity="info" rounded>WebGL Vendor:</Tag>
            {{ info?.WebGLVendor }}
          </div>
          <div class="col-4">
            <Tag severity="info" rounded>WebGL Renderer:</Tag>
            {{ info?.WebGLRenderer }}
          </div>
          <div class="col-4">
            <Tag severity="info" rounded>Mouse Position:</Tag>
            {{ info?.TrackMouse }}
          </div>
          <div class="col-4">
            <Tag severity="info" rounded>Network Information:</Tag>
            {{ info?.NetworkInformation.EffectiveType }}
          </div>
          <div class="col-4">
            <Tag severity="info" rounded>ScreenOrientation:</Tag>
            {{ info?.ScreenOrientation }}
          </div>
          <div class="col-6">
            <Fieldset legend="Chrome Plugins">
              <Tag
                v-for="item in info.Plugins"
                :key="item"
                class="ml-3"
                severity="info"
                rounded
          ...
          <div class="col-6">
            <Fieldset legend="Language List">
              <Tag
                v-for="item in info.LanguageList"
                :key="item"
                class="ml-3"
                severity="info"
                rounded
          ...
          <div class="col-12">
            <Fieldset legend="MediaDevices">
              <Tag
                v-for="item in info.MediaDevices"
                :key="item"
                class="ml-3"
                severity="info"
                rounded
          ...
          <div class="col-12">
            <Fieldset legend="Hardware Information">
              <label
                class="mb-3"
                v-for="item in info.HardwareInformation"
                :key="item"
              >
                <Tag class="ml-3" severity="info" rounded>
                  {{ item.name }}:
                </Tag>
                {{ item.value }}
              </label>
